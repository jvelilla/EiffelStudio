...
feature {NONE} -- User interface initialization
		...
		do
			...
			if class_list /= Void and then not class_list.is_empty then
				...
				if l_tabbable /= Void then
					...
					-- Size size info				
				l_list.set_minimum_width (development_window.scaled_size (300))
				l_list.set_column_count_to (1)
				l_list.column (1).set_width (development_window.scaled_size (300))
				l_list.set_row_height (18)
				l_list.set_minimum_height (class_list.count.min (14) * development_window.scaled_size (18))
	...
	populate_row (a_class: CLASS_I; a_row: EV_GRID_ROW)
		...
		do
			...
			l_item.set_tooltip (a_class.file_name.name)
