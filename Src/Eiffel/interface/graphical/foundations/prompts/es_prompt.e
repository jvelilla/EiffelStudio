...
feature {NONE} -- User interface initialization
	...
	frozen build_dialog_interface (a_container: EV_VERTICAL_BOX)
		...
		do
			...
			l_hbox.set_padding (development_window.scaled_size ({ES_UI_CONSTANTS}.prompt_horizontal_icon_spacing))
			l_hbox.extend (l_vbox)
			l_hbox.disable_item_expand (l_vbox)

			l_container.set_padding (development_window.scaled_size ({ES_UI_CONSTANTS}.prompt_vertical_padding))
			...
			create l_cell
			l_cell.set_minimum_width (development_window.scaled_size (25))
	...
	set_label_text (a_label: EVS_LABEL; a_text: READABLE_STRING_GENERAL)
		...
		do
			if a_text.is_empty then
				...
				if is_shown then
					l_old_width := development_window.scaled_size (dialog.width)
					l_old_height := development_window.scaled_size (dialog.height)
					a_label.set_maximum_width (development_window.scaled_size (maximum_prompt_text_width))
				end
				...
				if not a_label.is_displayed then
					...
					l_width := development_window.scaled_size (prompt_text.font.string_width (prompt_text.text))
					a_label.set_maximum_width (l_width.min (development_window.scaled_size (maximum_prompt_text_width)))
					...
					l_dialog := dialog
					l_width := development_window.scaled_size (l_dialog.width)
					l_height := development_window.scaled_size (l_dialog.height)
	...
	on_before_show
		...
		do
			Precursor {ES_DIALOG}
				...
				-- This will cause long text to be wrapped.
			l_width := development_window.scaled_size (prompt_text.font.string_width (prompt_text.text))
			prompt_text.set_maximum_width (l_width.min (development_window.scaled_size (maximum_prompt_text_width)))
			prompt_text.calculate_size

			l_width := development_window.scaled_size (prompt_sub_title_label.font.string_width (prompt_sub_title_label.text))
			prompt_sub_title_label.set_maximum_width (l_width.min (development_window.scaled_size (maximum_prompt_text_width)))
